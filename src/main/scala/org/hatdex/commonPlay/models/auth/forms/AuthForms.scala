...
object AuthForms {
  ...
  def signUpForm(implicit messages: Messages): Form[User] = Form(userMapping(messages))

  def userMapping(implicit messages: Messages): Mapping[User] = mapping(
    ...
    "terms" -> checked(Messages("signup.termsAgree.mandatory")))({
      // APPLY
